...
from UN_encoder import UNHyperpriorEncoder
...
def save_checkpoint(state, is_best, step):
    ...
    if not os.path.exists(root):
        os.mkdir(root)
    filename = os.path.join(root, "epoch_{}_step_{}_layer{}_loss_{:.6f}.pth.tar".format(state["epoch"], step, NoiseLayerNum, state["loss"]))
    torch.save(state, filename)
    if is_best:
        print("New Best Checkpoint Saved!")
        shutil.copyfile(filename, os.path.join(root, "checkpoint_layer{}_best_loss.pth.tar".format(NoiseLayerNum)))
    print("\n")
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-nl", "--NoiseLayerNum", type=int, required=True, help="The convolution layer with uniform noise added"
    )
    parser.add_argument(
        "-e",
        "--epochs",
...
def main(argv):
    ...
    global train_dataloader, test_dataloader, criterion, lr_scheduler, best_loss, subroot, NoiseLayerNum
    subroot = args.subroot
    NoiseLayerNum = args.NoiseLayerNum
    ...
    net = UNHyperpriorEncoder(prefpnet, NoiseLayerNum = args.NoiseLayerNum)
    # fpstate = {}
    ...
    print("===The Uniform Noise Parameters to Update===")
    print(parameters)
    params_dict = dict(net.named_parameters())
    optimizer = optim.Adam(
        (params_dict[n] for n in sorted(parameters)),
    ...
    if args.checkpoint:  # load from previous quantized checkpoint
        ...
        if int(args.checkpoint.split("_")[1][-1]) == NoiseLayerNum:
            best_loss = checkpoint["loss"]
        else:
            checkpoint["state_dict"]["g_a_noise." + str(NoiseLayerNum) + ".UNoise_n"] = net.state_dict()["g_a_noise." + str(NoiseLayerNum) + ".UNoise_n"]
