...
from LSQPlus_yMSEsMSE_model import LSQPlusScaleHyperprior
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        out["msescl_loss"] = self.msescl(output["scales_hat"], output["scales_hat_fp"])
        out["rd_loss"] = self.lmbda * 255**2 * out["mse_loss"] + out["bpp_loss"]
        out["loss"] = out["rd_loss"] + out["msey_loss"] * 1e-1
        msescl_lmbda = 1e-1
        out["loss"] += out["msescl_loss"] * msescl_lmbda
...
def test_epoch(epoch, model):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            msescl_loss.update(out_criterion["msescl_loss"])
            loss.update(out_criterion["loss"])
            rd_loss.update(out_criterion["rd_loss"])
            mse_loss.update(out_criterion["mse_loss"])

    print(
        f"Test epoch {epoch}: Average losses:"
        f"\tLoss: {loss.avg:.3e} |"
        f"\tRD Loss: {rd_loss.avg:.3e} |"
        f'\tPSNR: {cntPSNR(mse_loss.avg):.3f} |'
        f"\tMSE loss: {mse_loss.avg:.3f} |"
        f"\tMSEy loss: {msey_loss.avg:.2e} |"
        f"\tMSEscl loss: {msescl_loss.avg:.2e} |"
