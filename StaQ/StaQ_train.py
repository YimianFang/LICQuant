...
from StaQ_model import StaQScaleHyperprior
...
def train_one_epoch(
    model,  optimizer, aux_optimizer, epoch, clip_max_norm, save, method
):
    global best_loss
    model.train()
...
def save_checkpoint(state, is_best, step, method):
    root = "./checkpoint/StaQ_" + method
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--method", type=str, default="avg", help="The statistics method of QAT"
    )
    parser.add_argument(
        "-e",
        "--epochs",
...
def main(argv):
    ...
    net = StaQScaleHyperprior(prefpnet, method=args.method)
    # fpstate = {}
